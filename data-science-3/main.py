...
def q1():
    ...
    first_comp = pca.explained_variance_ratio_[0]

    return float(round(first_comp,3))
q1()
...
def q3():
    ...
    coordenadas = np.dot(pca.components_, x)
...
def q4():
    ...
    df = pd.DataFrame({'coluna':X_train.columns,
              'bool': rfe.get_support(),
              'coeficientes': pd.Series(reg.coef_)})

    return list(X_train.columns[df['bool']])
